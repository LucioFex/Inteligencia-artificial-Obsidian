...
var Packup4AIPlugin = class extends Plugin {
  constructor() {
    ...
    __publicField(this, "d3", null);
  }
  // Will be loaded in onload()
  onload() {
    return __async(this, null, function* () {
      try {
        const d3Module = yield Promise.resolve().then(() => (init_src32(), src_exports));
        this.d3 = d3Module;
      } catch (error) {
  ...
  renderVisualization() {
    return __async(this, null, function* () {
      ...
      try {
        ...
        const svg2 = d32.create("svg").attr("viewBox", [0, 0, width, height]);
        ...
        const sizeScale = d32.scaleSqrt().domain([Math.max(0, minWords), Math.max(1, maxWords)]).range([5, 15]);
        const simulation = d32.forceSimulation(nodes).force("link", d32.forceLink(links).id((d) => d.id).distance(100)).force("charge", d32.forceManyBody().strength(-250)).force("center", d32.forceCenter(width / 2, height / 2));
        ...
        const node = svg2.append("g").selectAll("circle").data(nodes).join("circle").attr("r", (d) => sizeScale(Math.max(0, d.wordCount))).attr("fill", (d) => d32.schemeCategory10[d.depth % 10]).attr("stroke", "#fff").attr("stroke-width", 1.5).call(this.dragBehavior(simulation));
...
${d.wordCount.toLocaleString()} words`);
        ...
        const zoomBehavior = d32.zoom().extent([[0, 0], [width, height]]).scaleExtent([0.2, 8]).on("zoom", (event) => {
          svg2.selectAll("g").attr("transform", event.transform);
        });
        svg2.call(zoomBehavior);
        svg2.call(zoomBehavior.transform, d32.zoomIdentity.scale(1.2));
